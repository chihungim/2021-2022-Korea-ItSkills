...
public class DB {
	...
	public DB() {
		...
		execute("grant select, insert, update, delete on 2022지방_2.* to user@localhost");
		...
		try {
			...
			while (rs.next()) {
				...
				pst.setBinaryStream(1, new FileInputStream("./datafiles/회원사진/" + rs.getInt(1) + ".jpg"));
		...
		try {
			...
			while (rs.next()) {
				...
				pst.setBinaryStream(1, new FileInputStream("./datafiles/기업/" + rs.getString(2) + "1.jpg"));
				pst.setObject(2, rs.getString(1));
				pst.execute();
			}
		} catch (SQLException | IOException e) {
